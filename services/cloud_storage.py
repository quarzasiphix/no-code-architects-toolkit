...
class S3CompatibleProvider(CloudStorageProvider):
    ...
    def upload_file(self, file_path: str) -> str:
        # Use MinIO Python SDK (minio==7.x) to push the file
        # object_name will live under "transcribe/{job_id}/<filename>"
        filename   = os.path.basename(file_path)
        job_id     = os.path.splitext(filename)[0]
        object_name = f"transcribe/{job_id}/{filename}"
        ...
        if not client.bucket_exists(self.bucket_name):
            client.make_bucket(self.bucket_name, location=self.region or None)

        # Upload the file
        client.fput_object(self.bucket_name, object_name, file_path)

        # Return the public URL
        scheme = "https" if self.endpoint_url.startswith("https") else "http"
        return f"{scheme}://{self.endpoint_url}/{self.bucket_name}/{object_name}"
